...
<body>
    ...
    <script>
        let isLanguageSwitching = false; // Dil değişikliği için bayrak
        ...
        function showModal(message) {
            if (isLanguageSwitching) {
                isLanguageSwitching = false; // Dil değişikliği olduğunda modal'ı gösterme
                return;
            }
        ...
        function switchLanguage(lang) {
            isLanguageSwitching = true; // Dil değişikliği sırasında modal'ı engellemek için bayrağı ayarla
        ...
        document.querySelector("form").onsubmit = function(event) {
            ...
            .then(data => {
                ...
                if (data.status === "error") {
                    ...
                    alert(data.message); // For successful screenshot, use alert or other notification
                }
            })
            .catch(error => {
...
func screenshotHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Respond with success message without triggering modal
